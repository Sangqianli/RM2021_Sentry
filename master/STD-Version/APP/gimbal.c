...
float yaw_PPM_ratio=1;
float yaw_PVM_ratio=4;
float pitch_PVM_ratio=2;
float pitch_PPM_ratio=1;//陀螺仪0.2
float yaw_error;
float yaw_round;
float pitch_base;
...
void Gimbal_Init()
{
    ...
    Gimbal_yaw_PPM.PID_PPM.kp=15;//20
    ...
    Gimbal_yaw_PVM.PID_PVM.kp=16;//
    Gimbal_yaw_PVM.PID_PVM.ki=0.3;//
    ...
    Gimbal_pitch_PVM.PID_PVM.kp=13;//机械4
    Gimbal_pitch_PVM.PID_PVM.ki=0.6;//机械0.2
    ...
    Gimbal_pitch_PPM.PID_PPM.kp=15;//机械15
    ...
    if(Remote_Mode)
    {
        ...
        if(pitch_target_now>0)
        {
            if((pitch_target_now-pitch_target_last)>0)
            {
                ...
                if(Gimbal_pitch_PPM.position_set>400)
                {
                    Gimbal_pitch_PPM.position_set=400;
                }
...
//	  Gimbal_pitch_PPM.PID_PPM.measure=roll;
}
...
void Gimbal_line()
{
	Gimbal_pitch_PPM.PID_PPM.target=constrain(Gimbal_pitch_PPM.PID_PPM.target,-500.f,400.f);
//	if(Gimbal_pitch_PPM.PID_PPM.measure>0)
//	  Gimbal_yaw_PPM.PID_PPM.target=constrain(Gimbal_yaw_PPM.PID_PPM.target,-1987.f,1987.f);
//	else
	Gimbal_yaw_PPM.PID_PPM.target=constrain(Gimbal_yaw_PPM.PID_PPM.target,-3000.f,3000.f);
}
