...
static void Fire_Judge()
{
    ...
    Fly_time=Vision_process.data_kal.DistanceGet_KF/20;//30
    ...
    if(sys.remote_mode == AUTO)
    {
        if((sys.auto_mode == AUTO_MODE_ATTACK)
                &&(Vision_process.speed_get * Vision_process.data_kal.YawGet_KF <= 0)
                &&((abs(motor[GIMBAL_YAW].info->angle_sum - Vision_process.data_kal.YawGet_KF) <= abs(Real_Speed*Fly_time))|| (abs(Vision_process.predict_angle)<1))
...
void Fire_Init()
{
    ...
    Fire_process.PVM.kp=6;//
    Fire_process.PVM.ki=0.1;//
