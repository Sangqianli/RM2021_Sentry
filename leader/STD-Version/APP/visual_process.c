...
float predict_angle=0,k_pre=0.10f,predict_angle_raw=0,use_predic=1.1f;		/*yaw预测超前角度,预测参数0.1*/	float feedforward_angle=0,k_ff=1.f,use_ff=1; 	/*前馈角 -- 加速度*/
...
//	static uint16_t predict_delay_cnt=0;/*预测的延时函数*/
...
//	VisualProcess.PitchTarget_KF=KalmanFilter(&kalman_targetPitch,PitchTarget_now);
    ...
    if(predict_flag==true)
    {
        ...
        feedforward_angle = k_ff * target_accel_raw; 	/*计算前馈角*/
